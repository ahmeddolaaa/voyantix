...
- Voyage page laytime view: **Timeline B (day grid — one row per day, one cell per hour, counted / running total / status per day)** is the main view, with **C (cumulative laytime curve against the allowance)** as a small card above it. A (swimlanes) rejected.
- New "Cargo & loading" view per port call: cargo plan progress by cargo/mill, shift log with cargo filter, per-crane totals, stoppages while loading, and a **"Will it finish in laytime?" forecast card** (Adel approved 2026-09-24): pace = tonnes loaded ÷ time since loading commenced; finish = remaining tonnes ÷ pace; compared with the wall-clock instant laytime runs out (engine, honouring excepted periods); shows needed pace. Estimate only — never feeds a statement or claim. Shifts are dated, not timed, so pace is whole-period, not per shift. No schema change.
