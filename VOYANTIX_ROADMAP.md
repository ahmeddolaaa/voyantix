...
**Where we stopped (2026-09-25 afternoon):**
- ✅ Production backup 2026-09-24 (`~/voyantix/voyantix-backup-2026-09-24T15-46-51-168Z.json` in Cloud Shell).
- ✅ Wipe + first demo seed ran on production 2026-09-25 ("Bulk Trading").
- ✅ **Postgres password rotated** 2026-09-25: `ALTER USER postgres` through the tunnel + Railway Postgres `POSTGRES_PASSWORD` (PGPASSWORD / DATABASE_URL reference it; voyantix `DATABASE_URL` references Postgres) → voyantix redeployed, site logs in. `.env.backup` and the tunnel are gone. Nobody holds the new password outside Railway — do NOT reintroduce a local DATABASE_URL; run DB scripts inside the service with `railway ssh`.
- ✅ **All demo names made fictional** (Adel: "no real data at all"). "Bulk Trading" is a real Swiss firm, NORTHERN GRACE a real ship, Kestrel a real agency, Quay 85/86 + rebar from Alexandria pointed at Dekheila. Now: company **Voyantix Demo** (`ahmed@` / `karim@voyantix-demo.example` — `.example` is a reserved domain), vessels CAPE HALDEN, SEA LANTERN, TALLIS REACH, AMBERLEY SOUND, HESPER VALE, IRON WREN, QUILLON BAY, MARRICK POINT, ORION PEAK, ATLAS DAWN; charterers Tessaline Chartering / Brisewood Maritime / Corvell Bulk; berths A1/A2 (Alexandria), B1, C1, D1, E1; cargoes Wheat, Urea (granular), Clinker, Steel products; SOF agent **Lanternhill Shipping Agency**, cargo clinker. Alexandria term now ends laytime at OPS_COMPLETED (no lashing for bulk). Ports stay real (public geography — Adel's choice). The video should carry "All data shown is fictional".
- ⏳ **Re-seed production with the fictional names** (commit "demo: fully fictional names"): after it deploys, in Cloud Shell `railway ssh --service voyantix`, then inside the container `WIPE_CONFIRM=wipe-everything DEMO_PASSWORD='<Adel's>' npm run demo:seed`.

**Next steps, in order:**
1. Re-seed as above. Login: **ahmed@voyantix-demo.example** / DEMO_PASSWORD.
2. **Gemini live check:** MV ATLAS DAWN → Import from SOF → `video-assets/SOF_MV_ATLAS_DAWN.pdf`. Verified locally with a stand-in: 7 events, 3 stoppages, actual 7,512.34 MT, recalculated → EXCEEDED by ≈5h 04m → **demurrage ≈ 2,533.97**.
3. **Reports — decision pending from Adel:** build v1 (claims by voyage / counterparty / port, laytime used vs allowed per port call, tonnes by month/port, CSV export) or hide it from the sidebar.
4. `video-assets/RECORDING-CHECKLIST.md` is **stale** — rewrite it for the Voyantix Demo data, then the LinkedIn video.
...
- **Login (demo):** `ahmed@voyantix-demo.example` / DEMO_PASSWORD (Adel's) — org "Voyantix Demo" (slug `voyantix-demo`).
...
4. ~~Vision-LLM extraction~~ — **DONE 2026-09-24**: upload (PDF / PNG / JPG / WEBP, ≤15 MB; server-action body limit 16 MB) → `extractSofDocument` sends it inline to Gemini `generateContent` (x-goog-api-key = Railway `GEMINI_API_KEY`; model `GEMINI_MODEL` or gemini-3.5-flash → gemini-3.8-flash on 404; JSON schema via responseJsonSchema, retried without it on 400) → `normalizeExtraction` validates every row against the SofExtraction contract (bad rows dropped and listed in conflicts) → existing review → commit. Nothing is stored until commit; the document is not kept. The MY FELLAS fixture is no longer reachable from the UI (tests only). **Free tier: Google may use the content to improve its products → demo documents only; paid tier before any real customer SOF.**
...
**Redesign 2026-09-24 (approved by Adel on the Design canvas "Voyantix — Redesign"):**
- Positioning is wider than laytime: sign-in headline "From berth to bill — every voyage in one place." / "Live while it loads. Settled when it sails." Product = cargo plan + live loading + laytime & claims.
- App frame: dark left sidebar (Dashboard, Voyages, Contracts, Reports; Setup: Rule sets, Master data & settings) replaces the top bar. Only real routes are listed; items a role cannot open are hidden. **Shipped** (bundle `voyantix-frame`).
- Voyage page laytime view (**shipped 2026-09-24**, `lib/laytime/visual/day-grid.ts`): **Timeline B (day grid — one row per day, one cell per hour, counted / running total / status per day)** is the main view, with **C (cumulative laytime curve against the allowance)** as a small card above it. A (swimlanes) rejected.
- New "Cargo & loading" view per port call: cargo plan progress by cargo/mill, shift log with cargo filter, per-crane totals, stoppages while loading, and a **"Will it finish in laytime?" forecast card** (Adel approved 2026-09-24): pace = tonnes loaded ÷ time since loading commenced; finish = remaining tonnes ÷ pace; compared with the wall-clock instant laytime runs out (engine, honouring excepted periods); shows needed pace. Estimate only — never feeds a statement or claim. Shifts are dated, not timed, so pace is whole-period, not per shift. No schema change. **Shipped 2026-09-24** (`lib/laytime/forecast.ts` pure + 4 tests; run-out instant = bisection on the provisional engine's used(t), to the minute, 60-day horizon; `getLoadingOverview` action; Cargo & loading tab).
...
| F27 | Visual identity **"Chart room"** (redesign approved by Adel 2026-09-24, replaces the teal/gold/Inter set): deep-sea ink `#0B2A33` sidebar + primary actions, chart-paper canvas `#F4F1EA`, brass `#B8862F` accent, teal `#1E6F69` = laytime / on track, coral `#C4472F` = demurrage / destructive, amber `#B7791F` = warning, slate `#51607E` = stoppages. Space Grotesk display, IBM Plex Sans UI, IBM Plex Mono figures. Tokens live in `app/globals.css` (legacy names kept, values remapped). Mockups for every screen: Design canvas "Voyantix — Redesign" | Adel 2026-09-24 | 8 | 🔄 frame shipped, screens in progress |
...
| 2026-09-24 | Detailed laytime sheet in the statement document (Adel: the statement lacked the calculation details his sheets carry). Per port call, modelled on the MY FELLAS sheet: header (port, operation, C/P + date, cargo), events with weekday + "Laytime commenced" + the laytime-end event marked, terms in words, time-sheet (Date/From/To/Comments/Time to count, sheet wording via `lib/laytime/sheet-comments.ts`), totals (dd hh mm + days) and rate/value. New read action `getPortCallSheet` (persisted data only). A sheet is shown only when it is the calculation the statement line was built from. Prints as page 2+ | Adel | 8/9 | milestone | Printed MY FELLAS sheet matches his PDF line by line |
| 2026-09-24 | Demo-readiness step 2 — SOF stoppages now land: when no org stoppage reason matches, Commit creates one under the reviewed category's neutral name (Labour break, Port closure, Religious…; Weather flagged) — countability stays a contract rule (F11). The commit summary lists created reasons and the reasons the term has no rule for yet ("Next: set Stoppage rules"). The STOPPAGE_RULE_MISSING refusal now names the reason and where to fix it (refusal carries a `subject`; compute rewords it). MY FELLAS replica: 8 stoppages committed (the port closure overlapping an already-committed labour break is rejected by PO13 and reported), rules added → still $13,537.82 | Adel (demo video goal) | 8 | milestone | Which of two overlapping SOF stoppages is kept follows commit order (PO13 frozen) |
| 2026-09-24 | Demo-readiness step 3 — statement sheet completed to Adel's layout: (a) SOF record-only events (vessel arrived, NOR re-tendered) are committed under ordinary custom event types created on first use (no engine semantic, F8) and appear on the sheet; (b) a row spanning the berthing is shown as two lines, rows before berth read "waiting for berthing"; (c) once on demurrage, consecutive counted rows of the same day merge into one (as the sheets do) — rows that do not count stay separate; (d) term field `laytime_clause_text` (migration 0021) = the C/P wording (e.g. "3000 MT PWWD FSHEX EIU"), DISPLAY ONLY, printed as "Loading rate"; (e) stoppage links are now saved for every interval inside a stoppage (also when it counts on demurrage) so the sheet can name the reason. MY FELLAS sheet matches the PDF row for row except the PDF's extra 05:40 shifting split | Adel | 8 | milestone | |
