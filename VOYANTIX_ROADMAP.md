...
3. **Despatch calculation** — `lib/laytime/settlement.ts` still refuses any despatch (`DESPATCH_BASIS_WITHHELD`). Basis can now be stored (WTS/ATS); WTS computation not built. test_2 has a real despatch example (3d 11h 26m saved × $4,375 → $15,211.76) — **the test_2 file itself is needed** (not in the repo) to derive the WTS measure exactly; MV YUFIX has no despatch.
